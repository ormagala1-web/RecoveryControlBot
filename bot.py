...
VERSION = "1.1.4 — ARCHIVOS PRIMERO Y NAVEGACIÓN CERTIFICADA"
...
def _campo_respaldo(respaldo, nombre: str, defecto=None):
    try:
        if nombre in respaldo.keys():
...
def correlativo_visible_respaldo(respaldo) -> str:
    valor = _campo_respaldo(respaldo, "numero_bot", _campo_respaldo(respaldo, "id"))
    try:
        return f"R{int(valor):03d}"
    except Exception:
        return "R---"


def version_visible_respaldo(respaldo) -> str:
    version = str(_campo_respaldo(respaldo, "version", "") or "").strip()
    return f"v{version}" if version else "SIN VERSION"


def estado_version_visible_respaldo(respaldo) -> str:
    return str(_campo_respaldo(respaldo, "version_estado", "LEGADO") or "LEGADO").strip().upper()


def fuente_visible_respaldo(respaldo) -> str:
    fuente = str(_campo_respaldo(respaldo, "fuente", "") or "").strip().upper()
    if fuente:
        return fuente
    tipo = str(_campo_respaldo(respaldo, "tipo", "") or "").strip().upper()
    return {"FUENTE_GITHUB":"GITHUB","FUENTE_JUSTRUNMY":"JUSTRUNMY","FUENTE_ORACLE":"ORACLE","MANUAL":"COMPLETO","MASIVO":"COMPLETO"}.get(tipo, tipo or "DESCONOCIDA")


def fecha_visible_respaldo(respaldo) -> str:
    fecha_peru = str(_campo_respaldo(respaldo, "fecha_peru", "") or "").strip()
    if fecha_peru:
        return fecha_peru
    return formatear_fecha_historial(_campo_respaldo(respaldo, "fecha_creacion"))


def referencia_visible_respaldo_id(respaldo_id) -> str:
    try:
        respaldo = obtener_respaldo(int(respaldo_id))
    except Exception:
        respaldo = None
    return correlativo_visible_respaldo(respaldo) if respaldo else "R---"


def resumen_visible_respaldo(respaldo) -> str:
    if not respaldo:
        return "R--- · SIN VERSION · DESCONOCIDA · Sin fecha"
    return (
        f"{correlativo_visible_respaldo(respaldo)} · "
        f"{version_visible_respaldo(respaldo)} · "
        f"{fuente_visible_respaldo(respaldo)} · "
        f"{fecha_visible_respaldo(respaldo)}"
    )
...
def teclado_historial_bot(bot_id: int) -> InlineKeyboardMarkup:
    ...
    for respaldo in reversed(respaldos):
        fecha = fecha_visible_respaldo(respaldo)
        correlativo = correlativo_visible_respaldo(respaldo)
        version = version_visible_respaldo(respaldo)
        fuente = fuente_visible_respaldo(respaldo)
        filas.append(
            [
                InlineKeyboardButton(
                    f"📦 {correlativo} · {version} · {fuente} · {fecha}",
...
def texto_detalle_respaldo(respaldo_id: int) -> str:
    ...
    fecha = html.escape(fecha_visible_respaldo(respaldo))
    correlativo = html.escape(correlativo_visible_respaldo(respaldo))
    version = html.escape(version_visible_respaldo(respaldo))
    version_estado = html.escape(estado_version_visible_respaldo(respaldo))
    fuente = html.escape(fuente_visible_respaldo(respaldo))
    ...
    return (
        ...
        f"📦 Correlativo: <code>{correlativo}</code>\n"
        f"🏷 Versión: <b>{version}</b>\n"
        f"🧭 Fuente: <b>{fuente}</b>\n"
        f"✅ Estado versión: <b>{version_estado}</b>\n"
...
async def descargar_respaldo(
    ...
    with ruta.open("rb") as archivo:
        documento = await context.bot.send_document(
            ...
            caption=(
                f"📦 {correlativo_visible_respaldo(respaldo)} · {version_visible_respaldo(respaldo)}\n"
...
def teclado_respaldos_restauracion(bot_id: int) -> InlineKeyboardMarkup:
    ...
    # Los archivos siempre se muestran primero:
    # antiguos arriba y nuevos abajo.
    for respaldo in reversed(respaldos):
        ...
        filas.append([
            InlineKeyboardButton(
                f"{etiqueta} {correlativo} · {version} · {fecha}",
                callback_data=f"respaldo_restaurar:{respaldo['id']}",
            )
        ])

    # Las acciones de gestión quedan debajo de la lista de archivos.
...
def teclado_fuentes_respaldo(bot_id: int) -> InlineKeyboardMarkup:
    bot = obtener_bot(int(bot_id))
    etiqueta_oracle = "☁️ Oracle · proyecto local"

    if bot:
        estado = estado_visual_fuente(bot, "oracle")
        if estado.get("codigo") == "NO_APLICA":
            etiqueta_oracle = "☁️ Oracle · no aplica"
        elif estado.get("codigo") == "CONFIGURACION_PENDIENTE":
            etiqueta_oracle = "☁️ Oracle · configurar ruta"
        elif estado.get("disponible"):
            etiqueta_oracle = "☁️ Oracle · disponible"
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(etiqueta_oracle, callback_data=f"source_panel:oracle:{bot_id}")],
...
def texto_fuentes_respaldo(bot) -> str:
    return (
        ...
        "☁️ <b>Oracle</b>: solo aplica cuando existe una instalación local real del bot en este servidor.\n\n"
...
def estado_disponibilidad_fuente(bot, fuente: str) -> tuple[bool, str]:
    ...
    if fuente == "oracle":
        ...
        return bool(estado.get("disponible")), str(
            estado.get("mensaje") or "Oracle no disponible."
        )

    return False, "Fuente desconocida."


def estado_visual_fuente(bot, fuente: str) -> dict:
    fuente = str(fuente or "").lower()

    if fuente == "oracle":
        estado = dict(estado_fuente_oracle(int(bot["id"])) or {})
        codigo = str(estado.get("estado") or "").upper()

        if estado.get("disponible"):
            etiqueta = "✅ Disponible"
        elif codigo == "NO_APLICA":
            etiqueta = "➖ No aplica"
        elif codigo == "CONFIGURACION_PENDIENTE":
            etiqueta = "⚠️ Configuración pendiente"
        else:
            etiqueta = "⚠️ No disponible"

        return {
            "disponible": bool(estado.get("disponible")),
            "codigo": codigo or "NO_DISPONIBLE",
            "etiqueta": etiqueta,
            "detalle": str(estado.get("mensaje") or "Oracle no disponible."),
        }

    disponible, detalle = estado_disponibilidad_fuente(bot, fuente)
    return {
        "disponible": disponible,
        "codigo": "DISPONIBLE" if disponible else "NO_DISPONIBLE",
        "etiqueta": "✅ Disponible" if disponible else "⚠️ No disponible",
        "detalle": detalle,
    }
...
def texto_panel_fuente(bot_id: int, fuente: str) -> str:
    ...
    estado_fuente = estado_visual_fuente(bot, fuente)
    disponible = bool(estado_fuente["disponible"])
    detalle = str(estado_fuente["detalle"])
    ultima_guardada = ultima_copia_fuente(bot_id, fuente, {"GUARDADO"})
    ultima_disponible = ultima_copia_fuente(bot_id, fuente, {"DISPONIBLE"})

    estado_txt = str(estado_fuente["etiqueta"])
    lineas = [
    ...
    if ultima_guardada:
        lineas.extend([
            ...
            f"• {resumen_visible_respaldo(ultima_guardada)}",
    ...
    if ultima_disponible:
        lineas.extend([
            ...
            f"• {resumen_visible_respaldo(ultima_disponible)}",
            "",
        ])

    if fuente == "oracle" and estado_fuente.get("codigo") == "NO_APLICA":
        lineas.append(
            "Esta fuente es informativa para este bot: no se ofrece creación de copia "
            "Oracle porque no existe una instalación local del proyecto en este servidor."
        )
    elif disponible:
        lineas.append(
        ...
        lineas.append(
            "La fuente no está operativa todavía. Revisa su configuración antes de crear copias."
        )
    return "\n".join(lineas)
...
def teclado_panel_fuente(bot_id: int, fuente: str) -> InlineKeyboardMarkup:
    ...
    if bot:
        estado = estado_visual_fuente(bot, fuente)
    ...
    elif fuente == "oracle" and estado.get("codigo") == "CONFIGURACION_PENDIENTE":
        filas.append([
            InlineKeyboardButton(
                "✏️ Configurar ruta Oracle",
                callback_data=f"bot_editar:{bot_id}",
            )
        ])
    elif not (fuente == "oracle" and estado.get("codigo") == "NO_APLICA"):
        filas.append([
            InlineKeyboardButton(
                "ℹ️ Fuente sin configurar",
...
def teclado_biblioteca_fuente(bot_id: int, fuente: str) -> InlineKeyboardMarkup:
    filas = []
    for respaldo in reversed(copias_fuente(bot_id, fuente, limite=30)):
        if str(respaldo["estado"] or "").upper() != "GUARDADO":
            continue
        filas.append([
            InlineKeyboardButton(
                f"{correlativo_visible_respaldo(respaldo)} · {version_visible_respaldo(respaldo)} · {fecha_visible_respaldo(respaldo)}",
...
def teclado_biblioteca_fuentes(bot_id: int) -> InlineKeyboardMarkup:
    filas = []
    for respaldo in reversed(respaldos_fuente_guardados(bot_id)):
        fecha = fecha_visible_respaldo(respaldo)
        ...
        filas.append([
            InlineKeyboardButton(
                f"{fuente} · {correlativo} · {version} · {fecha}",
                callback_data=f"source_saved_detail:{respaldo['id']}",
            )
        ])
    filas.append([
...
def es_membresias_consultas_denuncias_bot(bot) -> bool:
    ...
    return (
        ...
        or "membresias consultas y denuncias" in nombre
        or "membresías consultas y denuncias" in nombre
...
async def ejecutar_restauracion_completa(
    ...
    await query.edit_message_text(
        ...
        f"<b>{html.escape(referencia_visible_respaldo_id(preventivo.get('respaldo_id')))}</b>\n\n"
    ...
    for bot in obtener_bots():
        ...
        if respaldo:
            filas.append({
                ...
                "respaldo_ref": correlativo_visible_respaldo(respaldo),
                "version": version_visible_respaldo(respaldo),
...
def texto_plan_restauracion_masiva() -> str:
    ...
    for indice, item in enumerate(plan, start=1):
        ...
        if item["valido"]:
            lineas.append(
                f"✅ {indice}. {nombre} · <b>{html.escape(str(item['respaldo_ref']))}</b> · {html.escape(str(item['version']))}"
            )
        else:
            todos_validos = False
...
async def editar_progreso_masivo(query, titulo: str, lineas: list[str]) -> None:
    ...
    try:
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("⬅️ Operaciones masivas", callback_data="bulk_menu")],
                [InlineKeyboardButton("🏠 Inicio · el proceso continúa", callback_data="home")],
            ]),
        )
    except TelegramError:
        pass
    ...
    try:
        ...
        for indice, bot in enumerate(bots, start=1):
            ...
            if resultado.get("correcto"):
                ...
                lineas.append(f"✅ {indice}/{total} · {nombre} · {referencia_visible_respaldo_id(rid)}")
            else:
                fallidos.append(int(bot["id"]))
                lineas.append(
    ...
    try:
        ...
        for indice, bot in enumerate(bots, start=1):
            ...
            if resultado.get("correcto"):
                ...
                lineas.append(
                    f"✅ {indice}/{total} · {nombre} · {referencia_visible_respaldo_id(resultado.get('respaldo_id'))} · {base}"
                )
            else:
                fallidos.append(int(bot["id"]))
                lineas.append(
    ...
    for indice, bot in enumerate(bots, start=1):
        ...
        if respaldo:
            lineas.append(
                ...
                f"{correlativo_visible_respaldo(respaldo)} · {version_visible_respaldo(respaldo)} válido"
            )
        else:
            lineas.append(
                f"❌ {indice}/{total} · {_nombre_bot_corto(bot)} · "
        ...
        for indice, item in enumerate(plan, start=1):
            ...
            await query.edit_message_text(
                ...
                + (
                 ...
                 + f"⏳ {indice}/{total} · {html.escape(nombre)} · {html.escape(referencia_visible_respaldo_id(rid))}\n\n"
...
async def botones_generales(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if opcion.startswith("source_saved_detail:"):
        ...
        await query.edit_message_text(
            ...
            f"{etiqueta_fuente(respaldo['tipo'])}\n"
            f"🤖 <b>{html.escape(str(respaldo['bot_nombre'] or 'Bot'))}</b>\n"
            f"📦 {html.escape(correlativo_visible_respaldo(respaldo))} · {html.escape(version_visible_respaldo(respaldo))}\n"
            f"🧭 Fuente: <b>{html.escape(fuente_visible_respaldo(respaldo))}</b>\n"
            f"📄 <code>{html.escape(str(respaldo['archivo']))}</code>\n"
            f"📏 <b>{formatear_tamano(respaldo['tamano_bytes'])}</b>\n"
            f"🧾 Estado: <b>{html.escape(str(respaldo['estado'] or 'GUARDADO'))}</b>\n"
            f"🔐 <code>{html.escape(str(respaldo['sha256'] or ''))}</code>\n"
            f"🇵🇪 {html.escape(fecha_visible_respaldo(respaldo))}",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("⬇️ Descargar", callback_data=f"respaldo_descargar:{respaldo_id}")],
                [
    ...
    if opcion.startswith("respaldo_restaurar:"):
        ...
        try:
            ...
            await query.edit_message_text(
                texto_detalle_respaldo(respaldo_id)
                + "\n\n❌ <b>NO SE PUEDE PREPARAR LA RESTAURACIÓN</b>\n"
                + html.escape(str(error)),
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton(
                        "⬅️ Volver al respaldo",
                        callback_data=f"respaldo_detalle:{respaldo_id}",
                    )],
                    [
                        InlineKeyboardButton(
                            "📥 Restauración manual",
                            callback_data=f"bot_restore:{int(respaldo['bot_id'])}",
                        ),
                        InlineKeyboardButton("🏠 Menú principal", callback_data="home"),
                    ],
                ]),
